...
module.exports = function(commandsProcessor, vocabularyProcessor, uiDispatcher, settings, state) {
     ...
     */
    commandsProcessor.registerCommand('WritingTask', (scriptParser, ui, settings, state, params)=> {
        if(params.length == 1) {
    ...
    vocabularyProcessor.registerVocabularyFilter("SubWritingTaskMin",(settings,state,params)=>{
        return settings.sub.writingTask.min;
    });

    vocabularyProcessor.registerVocabularyFilter("SubWritingTaskMax",(settings,state,params)=>{
